  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  if (serial_fd_ < 0){
    RCLCPP_ERROR(get_logger(), "Failed to open serial port, continuing without serial communication.");
    //return hardware_interface::CallbackReturn::ERROR;
  }
  ...
  //hw_interfaces_["steering_right"] = Joint("front_right_steering_joint");

  hw_interfaces_["traction"] = Joint("virtual_rear_wheel_joint");
  //hw_interfaces_["traction_right"] = Joint("rear_right_steering_joint");
...
hardware_interface::return_type ros2_control_demo_example_11 ::CarlikeBotSystemHardware::write(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  double velocity = hw_interfaces_["traction"].command.velocity;
  double steering = hw_interfaces_["steering"].command.position;
  ...
  if (motor_cmd != last_motor_cmd){
    ::write(serial_fd_, &motor_cmd, 1);
    last_motor_cmd = motor_cmd;
  }
  if (steering_cmd != last_steering_cmd){
    ::write(serial_fd_, &steering_cmd, 1);
